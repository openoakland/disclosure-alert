...
class Election < ApplicationRecord
  ...
  def self.replace_all_from_csv(election_csv: ELECTION_CSV_URL,
    ...
    election_csv = URI.open(election_csv).read if election_csv.start_with?('http')
    election_deadlines_csv = URI.open(election_deadlines_csv).read if election_deadlines_csv.start_with?('http')
